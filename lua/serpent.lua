  ...
  local function s(t, opts)
    ...
    local alphanumsort = type(opts.sortkeys) == 'function' and opts.sortkeys or function(k, o, n) -- k=keys, o=originaltable, n=padding
      ...
      local function val2str(t, name, indent, insref, path, plainindex, level)
        ...
        if ttype == "table" then
        local so, sr = pcall(function() return mt.__serpent(t, function(x) return val2str(x) end) end)
        if so then
          return tag..sr
        end
...
if not maxnum or #o < maxnum then
        ...
        -- local pre_head = "setmetatable("
        local pre_head = ""
        local post_tail = ""
        -- local post_tail = "," .. s(getmetatable(t), opts) .. ")"
        return (custom and custom(tag, head, body, tail, level) or tag..pre_head..head..body..tail..post_tail)..comment(t, level)
      elseif ttype == "userdata" then
        seen[t] = insref or spath
      local so, sr = pcall(function() return t.__serpent(t, function(x) return val2str(x) end) end)
      if so then
        return tag..sr
      else
...
return {_NAME = n, _COPYRIGHT = c, _DESCRIPTION = d, _VERSION = v, serialize = s,
      load = deserialize,
      dump = function(a, opts) return s(a, merge({name = '_', compact = true, sparse = true, metatostring = true}, opts)) end,
